import { drawCircle, drawLine } from "./circle.js";
...
export function drawTree(v) {
    ...
    if (v.parent !== null) {
        const x1 = r * Math.cos(theta + Math.PI / 2)
        const y1 = r * Math.sin(theta + Math.PI / 2)
        const rp = 80 * v.parent.level;
        const thetap = -fact * v.parent.x;
        const x2 = rp * Math.cos(thetap + Math.PI / 2)
        const y2 = rp * Math.sin(thetap + Math.PI / 2)
